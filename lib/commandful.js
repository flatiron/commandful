    ...
    var entity = resource._resource.toLowerCase(),
        ...
        router.on(entity + ' edit', function() {
          ...
          if(process.argv.slice(2).length === 2 ) {
            logger.warn('id'.magenta + ' is required is edit')
            //
            // TODO: prompt y/n to list all ids
            //
          }
        ...
        router.on(entity + ' show :id', function(id) {
          resource.get(id, function(err, result){
            ...
            show(result.id, result, resource);
          });
        });
...
function create (resource) {
  ...
  //
  // Remark: id is excluded from resource schemas, that is probably wrong.
  //
  var id = {
    properties: {
      id: {
        pattern: /^[a-zA-Z\s\-]+$/,
        message: 'id must be only letters, spaces, or dashes',
        required: false
      }
    }
  };

  prompt.get(id, function(err, _id){
